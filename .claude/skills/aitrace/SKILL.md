...
1. 读 `docs/timeline/` 最新的记忆文件，继承上下文与待办池
2. 除非用户另有指定，取**最高优先级待办**作为本轮主任务（WIP = 1）
3. 开工即声明该项的 DoD（完成定义），自迭代型任务的 DoD 四件套：修复提交 + 回归测试 + 部署验收（哈希/版本）+ 教训标准化
4. 顺手核对快环护栏：daemon 在录制、工作树干净
...
- **变异验证**（自举的不动点检查）：对重要缺陷的回归测试，偶尔故意重新引入缺陷，确认测试会红——测试测试本身
- 一切自动化触发（收尾提醒等）必须带一次性 guard（marker / stop_hook_active），防自指无限回归
